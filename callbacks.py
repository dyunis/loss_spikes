...
class ModelPerturber(Callback):
    def __init__(self, perturb_epoch, scale=1.0, perturb_bottom_half=False, seed=0): 
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        if trainer.current_epoch == self.perturb_epoch:
            self.perturb_svd_(pl_module)

    @torch.no_grad()
    def perturb_svd_(self, model):
        ...
        # skip perturbing embedding layer as too much memory usage
        named_mat_params = named_mat_params[1:-1]
        for n, p in named_mat_params:
            mat = p
            if len(p.shape) > 2:
                mat = p.flatten(1)
            U, s, Vh = self.safe_svd(mat)

            # decompose into different rank components
            svd_comps = U[:, :, None] * s[None, :, None] * Vh[None, :, :]
            svd_comps = svd_comps.permute(1, 0, 2)  # reshape to rank first

            # perts per rank
            if self.perturb_bottom_half:
                to_leave = svd_comps[:int(0.5 * len(svd_comps))]
                to_pert = svd_comps[int(0.5 * len(svd_comps)):]
            else:
                to_pert = svd_comps[:int(0.5 * len(svd_comps))]
                to_leave = svd_comps[int(0.5 * len(svd_comps)):]

            pert = torch.normal(0.0, 1.0, size=to_pert.size(), generator=generator, device=p.device)
            pert = self.scale * (to_pert.norm(dim=(1, 2), keepdim=True) / pert.norm(dim=(1, 2), keepdim=True)) * pert
            new_portion = to_pert + pert
            new_portion = (to_pert.norm(dim=(1, 2), keepdim=True) / new_portion.norm(dim=(1, 2), keepdim=True)) * new_portion
            new_param = torch.cat([new_portion, to_leave], dim=0).sum(dim=0)
            if len(p.shape) > 2:
                new_param = new_param.reshape(p.shape)
            p.copy_(new_param)

    @torch.no_grad()
    def safe_svd(self, mat):
        try:
            U, s, Vh = torch.linalg.svd(mat, full_matrices=False)
        except:
            eps = 1e-10 * torch.eye(*mat.shape, device=mat.device)
            U, s, Vh = torch.linalg.svd(mat + eps, full_matrices=False)
        return U, s, Vh
