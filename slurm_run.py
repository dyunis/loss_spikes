...
def make_sbatch_script(wandb_dir, wandb_project, wandb_group, wandb_name, wandb_program, run_args, sbatch_args):
    ...
    num_cores = sbatch_args['cpus_per_task']
    ...
    # shell works for interactive jobs, but fails for batch jobs
    # so create two scripts inside slurm directory: one to deal with slurm args and launch apptainer exec, the second with the actual contents of sourcing environment inside container and launching python
    lines.extend([
        ...
        '',
        f'apptainer exec --mount type=bind,src=/scratch,dst=/scratch --mount type=bind,src=/share,dst=/share --nv /share/data/speech/Data/dyunis/envs/apptainer/lmc_svd.sif bash {stump_path}',
    ])
...
def add_args(parser):
    ...
    parser.add_argument('-p', '--partition', type=str, default='contrib-gpu')
    parser.add_argument('-c', '--cpus_per_task', type=int, default=1)
    parser.add_argument('-C', '--constraint', default='', type=str)
    parser.add_argument('--mail_user', type=str, default='dyunis@ttic.edu')
